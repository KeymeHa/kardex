$(".tablaInsumosNVenta").on("click", "button.agregarInsumo", function(){
				...
				$("#cajaToNuFa").append(
					'<div class="row" style="padding:5px 15px">'+
		              ...
		              '<div class="col-xs-4" style="padding-right:0px">'+
		               	'<div class="input-group" title="IVA" id="cajaIVA">'+
		              	'</div>'+
		              '</div>'+
		            '</div>'+
		            '<div class="row" style="padding:5px 15px">'+
...
function cajaDeIVA()
{
	...
	if ($("#cajaIVA")) 
	{
		$("#cajaIVA").children().remove();
	}

	var datos = new FormData();
	datos.append("traerIMP", 1);
	...
	$.ajax({
		...
		success: function(respuesta)
		{
			...
			$("#cajaIVA").append('<span class="input-group-addon"><i class="fa fa-th"></i></span>'+
				'<select class="form-control input-lg" id="iva" name="iva" required>')

			for (var i = 0; i < respuesta.length; i++) 
			{
				$("#iva").append('<option value="'+respuesta["valor"]+'">'+respuesta["valor"]+'% - '+respuesta["descripcion"]+'</option>')
			}
		}
	});
...
$(".tablaInsumosNVenta").on("draw.dt", function(){
...
$(".tablaInsumosNVenta").on("click", "button.agregarInsumo", function(){
    ...
    if( $("button.btnGuardarVnt").hasClass("btn-success") == false )
    {
    	$("button.btnGuardarVnt").addClass("btn-success");
    	$('button.btnGuardarVnt').attr("disabled", false);
    }
...
$(".formularioNuevaVenta").on("click", "a.quitarInsumo", function(){
...
function deshabilitarCrearNF(){
	...
	$("button.btnGuardarVnt").removeClass("btn-success");
	$("button.btnGuardarVnt").addClass("btn-default");
	$('button.btnGuardarVnt').attr("disabled", true);
...
$(".formularioNuevaVenta").on("change", "input.nuevaCantidadInsumo", function(){
...
$(".formularioNuevaVenta").on("change", "input.nuevoContenido", function(){
	sumarPreciosNF();
	listarInsumosNF();
})

$(".formularioNuevaVenta").on("change", "input.nuevoImpuesto", function(){
	sumarPreciosNF();
	listarInsumosNF();
})


$(".formularioNuevaVenta").on("change", "input.nuevoPrecioInsumo", function(){
...
$(document).ready(function() {
	//$("button.btnGuardarVnt").removeClass("btn-success");
    $(".formularioNuevaVenta").keypress(function(e) {
        if (e.which == 13) {
            return false;
        }
    });
