...
class TablaRequisiciones
{
	...
	public function mostrarTablaRq()
	{
	    ...
	    for ($i=0; $i < count($requisiciones); $i++) 
	    {
            ...
            if ($sw == 0) 
            {
            	$acciones = "<div class='btn-group'><div class='col-md-3'><button class='btn btn-success btnVerSoli' idRq='".$requisiciones[$i]["id"]."' title='Tramitar'><i class='fa fa-book'></i> Tramitar</button></div></div>";
            }
            else
            {

            	$acciones = "<div class='btn-group'><div class='col-md-3'><button class='btn btn-success btnVerRq' idRq='".$requisiciones[$i]["id"]."' title='Ver Requisición'><i class='fa fa-book'></i></button></div><div class='col-md-3'><button class='btn btn-warning btnEditarRq' idRq='".$requisiciones[$i]["id"]."' title='Editar'><i class='fa fa-pencil'></i></button></div><div class='col-md-3'><button class='btn btn-info btnReplicarRq' idRq='".$requisiciones[$i]["id"]."' title='Replicar'><i class='fa  fa-copy'></i></button></div><div class='col-md-3'><button class='btn btn-danger btnEliminarRq' idRq='".$requisiciones[$i]["id"]."' title='Eliminar Rq'><i class='fa fa-close'></i></button></div></div>";
            }
